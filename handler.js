    ...
    try {
        ...
        try {
            ...
            if (chat) {
                ...
                if (!('negrometro' in chat)) chat.negrometro = true
            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                        ...
                        if (data.exists)
                            m.reply(`{❗️} 𝐄𝐑𝐑𝐎𝐑𝐄`.trim(), data.jid)
                    }
                }
            }
            if (!opts['restrict'])
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        let nomeDelBot = global.db.data.nomedelbot || `🌟 𝐎𝐧𝐢𝐱 🌟`
                        ...
                        this.sendMessage(id, { 
                            ...
                            contextInfo:{ 
                               ...
                               externalAdReply: {
                                    "title": `${action === 'add' ? '𐌟𐌟𐌟𐌟𐌟𐌟𐌟𐌟𐌟🌟  𝕎𝕖𝕝𝕔𝕠𝕞𝕖  🌟' : '𐌟𐌟𐌟𐌟𐌟𐌟𐌟𐌟𐌟👋🏼  𝔾𝕠𝕠𝕕𝕓𝕪𝕖  👋🏼'}`,
                                    "previewType": "PHOTO", 
                                    "thumbnailUrl": ``, 
                                    "thumbnail": apii.data,
                                    "mediaType": 1
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        let nomeDelBot = global.db.data.nomedelbot || `𝐎𝐧𝐢𝐱 🌟`
                        let apii = await this.getFile(pp)
                        text = (action === 'promote' ? (chat.sPromote || this.spromote || conn.spromote || ```@user 𝕖̇ 𝕦𝕟 𝕤𝕦𝕡𝕣𝕖𝕞𝕠```) :
                            (chat.sDemote || this.sdemote || conn.sdemote || ```@user 𝕟𝕠𝕟 𝕖̀ 𝕡𝕚𝕦̀ 𝕦𝕟 𝕤𝕦𝕡𝕣𝕖𝕞𝕠```)).replace('@user', '@' + user.split('@')[0])
                        this.sendMessage(id, { 
                            text: text, 
                            contextInfo:{ 
                                mentionedJid:[user],
                               ...
                               serverMessageId: '', newsletterName: `${nomeDelBot}` },
                               externalAdReply: {
                                    "title": `${action === 'promote' ? '⚜️ 𝔽𝕒𝕤𝕖 𝕕𝕚 𝕤𝕦𝕡𝕣𝕖𝕞𝕒𝕫𝕚𝕒 ⚜️' : '➖ 𝔽𝕒𝕤𝕖 𝕕𝕚 𝕣𝕖𝕥𝕣𝕠𝕔𝕖𝕤𝕤𝕚𝕠𝕟𝕖 ➖'}`,
                                    "previewType": "PHOTO", 
                                    "thumbnailUrl": ``, 
                                    "thumbnail": apii.data,
                                    "mediaType": 1
...
export async function callUpdate(callUpdate) {
    ...
    for (let nk of callUpdate) {
        if (nk.isGroup == false) {
            if (nk.status == "offer") {
                // Blocca il contatto senza inviare alcun messaggio
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '⚠️ 𝑺𝒐𝒍𝒐 𝒑𝒆𝒓 𝒈𝒍𝒊 𝒐𝒘𝒏𝒆𝒓𝒔 ⚠️',
        owner: '⚠️ 𝑺𝒐𝒍𝒐 𝒑𝒆𝒓 𝒈𝒍𝒊 𝒐𝒘𝒏𝒆𝒓𝒔 ⚠️',
        mods: '𝐐𝐮𝐞𝐬𝐭𝐨 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐥𝐨 𝐩𝐨𝐬𝐬𝐨𝐧𝐨 𝐮𝐭𝐢𝐥𝐢𝐳𝐳𝐚𝐫𝐞 𝐬𝐨𝐥𝐨 𝐚𝐝𝐝𝐦𝐢𝐧 𝐞 𝐨𝐰𝐧𝐞𝐫 ⚙️',
        ...
        admin: '⚠️ 𝑺𝒐𝒍𝒐 𝒑𝒆𝒓 𝒂𝒅𝒎𝒊𝒏 ⚠️',
        botAdmin: '𝑰𝒍 𝒃𝒐𝒕 𝒏𝒐𝒏 𝒆̀ 𝒂𝒅𝒎𝒊𝒏, 𝒄𝒐𝒈𝒍𝒊𝒐𝒏𝒆 🤡',
